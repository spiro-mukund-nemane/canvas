  ...
  reducers: {
    setCurrentProject: (state, action: PayloadAction<string>) => {
      ...
      if (project) {
        ...
        state.projects = [project, ...state.projects.filter((p) => p.id !== action.payload)]
      }
    },
    setProjects: (state, action: PayloadAction<Project[]>) => {
      ...
      if (state.currentProject) {
        ...
        const currentProjectInNewList = action.payload.find((p) => p.id === currentProjectId)

        if (currentProjectInNewList) {
          state.projects = [currentProjectInNewList, ...action.payload.filter((p) => p.id !== currentProjectId)]
        } else {
...
export const { setCurrentProject, setProjects, clearCurrentProject } = projectSlice.actions
